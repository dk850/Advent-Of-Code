...
# then we fold the paper based on all the fold rules
for rule in fold_rules:
    paper = fold_paper(paper, rule[0], rule[1])
print("Final folded grid:")
display_grid(paper)
print()

# change the elements to something more visiible and of the same length so we can read it
for y in range(len(paper)):
    for x in range(len(paper[0])):
        paper[y][x] = str('@') if paper[y][x] != 0 else str('.')
print("Grid with uniform sized elements for easier reading:")
display_grid(paper)
