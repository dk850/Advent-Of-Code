...
def determine_localality(layout, seat_row, seat_col):
    ...
    # check seats above seat
    if not top:
        if not lSide:
            if layout[seat_row-1][seat_col-1] == '#':
                occupied_count += 1
        if layout[seat_row-1][seat_col] == '#':
            occupied_count += 1
        if not rSide:
            if layout[seat_row-1][seat_col+1] == '#':
                occupied_count += 1

    # check seats adjacent to seat
    if not lSide:
        if layout[seat_row][seat_col-1] == '#':
            occupied_count += 1
    if not rSide:
        if layout[seat_row][seat_col+1] == '#':
            occupied_count += 1

    # check seats under seat
    if not bottom:
        if not lSide:
            if layout[seat_row+1][seat_col-1] == '#':
                occupied_count += 1
